...
3. **Executive Summary** — dynamically generated per level from the signals in scope (see below)
...
The Executive Summary card at each hierarchy level is resolved by `generateExecSummary()` using a three-tier priority:

1. **Pre-authored briefs** (`summary.group_briefs`): If the portfolio JSON contains `summary.group_briefs`, the function looks up the current group name. At the top level, the key is `_top`. At a drilled sub-level, the key is `navStack[navStack.length - 1].key` — the group name that was clicked (a region name, territory name, etc.). If a matching brief is found, it is returned verbatim. These briefs are generated by the agent during the pipeline's executive summary step (SKILL.md section 7).
2. **Non-placeholder `summary.text`**: At the top level only, if `data.summary.text` does not match the `build_portfolio.py` placeholder pattern (`"<ScopeType> <scopeValue> contains"`), it is treated as an AI-authored summary and used directly.
3. **Dynamic signal-based fallback**: When neither of the above applies, the function dynamically generates a summary from the signals in scope. This is the backward-compatibility path for old portfolios that lack `group_briefs`. It cites the top 2 signals by score from distinct accounts, adds a coverage line, and optionally appends the top internal risk at the account-list level. Headlines are truncated to 80 characters.

The `groupBriefs` variable is initialized at the top of the script from `data.summary.group_briefs`, defaulting to an empty object for portfolios that lack the field.
